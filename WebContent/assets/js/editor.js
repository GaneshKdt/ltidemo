...
(function( $ ){
	...
	var methods = {
		...
		imageWidget: function(){
			//Class for Widget Handling the upload of fileNames
			...
			handlefileNameSelect = function(evt) {
    			var fileNames = evt.target.fileNames; // fileNameList object
				var output = [];
				for (var i = 0, f; f = fileNames[i]; i++) {
					//Loop thorugh all the fileNames
					if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ //Process only Images
						methods.showMessage.apply(this,["imgErrMsg_" + _idSuffix,"Invalid fileName type"]);
						continue;
					}
					var reader = new fileNameReader();
					reader.onload = (function(imagefileName){
						return function(e){
							//Render Thumnails
							...
							var image = $('<img/>',{
								src:e.target.result,
								title:escape(imagefileName.name)
			...
			chooseFromLocal.on('change', handlefileNameSelect);
